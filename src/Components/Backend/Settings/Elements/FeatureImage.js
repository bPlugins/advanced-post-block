...
const FeatureImage = ({ attributes, setAttributes, updateObj }) => {
	const { subLayout, isFImg, fImgFitting, isFImgLink, image = {} } = attributes;
	const { width = '100%', height = '60%' } = image || {};
	...
	return <PanelBody className='bPlPanelBody' title={__('Feature Image', 'advanced-post-block')}>
		...
		{isFImg && <>
			...
			{!subLayout.includes('overlay') && <>
				<UnitControl className='mt20' label={<>{__('Width:', 'advanced-post-block')} <HelpTooltip text={__('Define custom width for the post image.', 'advanced-post-block')} /> <Badge /></>} labelPosition='left' value={width} onChange={val => updateObj('image', val, 'width')} units={[pxUnit(), perUnit(), emUnit()]} />

				<UnitControl className='mt10' label={<>{__('Height:', 'advanced-post-block')} <HelpTooltip text={__('Define custom height for the post image.', 'advanced-post-block')} /> <Badge /></>} labelPosition='left' value={height} onChange={val => updateObj('image', val, 'height')} units={[pxUnit(), perUnit(), emUnit()]} />
		...
		<Notice status='premium' isIcon={true}>{__('Custom image sizes, lazy loading, and fallback image are available in the Premium version.', 'advanced-post-block')}</Notice>
	</PanelBody>
};
