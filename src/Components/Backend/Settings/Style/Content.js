...
import { Label, Background, BtnGroup, HelpTooltip, Badge } from '../../../../../../bpl-tools/Components';
...
const Content = ({ attributes, setAttributes }) => {
	...
	return <PanelBody className='bPlPanelBody' title={__('Content', 'advanced-post-block')} initialOpen={false}>
		...
		<Background label={<>{__('Hover Background', 'advanced-post-block')} <HelpTooltip text={__('Apply a color, image, or gradient to the post box.', 'advanced-post-block')} /> <Badge /></>} value={hoverContentBG} onChange={val => setAttributes({ hoverContentBG: val })} defaults={{ color: '' }} />
