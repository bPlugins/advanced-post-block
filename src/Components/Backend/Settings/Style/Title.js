...
import { ColorControl, BtnGroup, Typography, HelpTooltip } from '../../../../../../bpl-tools/Components';
...
const Title = ({ attributes, setAttributes, updateObj }) => {
	...
	return <PanelBody className='bPlPanelBody' title={__('Title', 'advanced-post-block')} initialOpen={false}>
		...
		<BtnGroup label={__('Text Align:', 'advanced-post-block')} value={textAlign} onChange={val => updateObj('title', val, 'styles', 'textAlign')} options={aligns?.filter(a => 'justify' !== a.value)} isIcon={true} size='small' />
		...
		<ColorControl label={__('Hover Color:', 'advanced-post-block')} value={hoverColor} onChange={val => updateObj('title', val, 'styles', 'hoverColor')} defaultColor='' />
